...
public class ExceptionHandler implements Thread.UncaughtExceptionHandler {
    ...
    public ExceptionHandler() {
        ...
        exceptionHandlers.put(UnsupportedClassVersionError.class, this::handleUnsupportedClassVersionError);
    }
    ...
    private void handleUnsupportedClassVersionError(Throwable e) {
        System.out.println("你尝试运行的类使用了比当前JVM版本更高的编译版本。");
        System.out.println("原始错误信息: " + e.getMessage());
        String message = e.getMessage();
        int majorVersion = -1;
        java.util.regex.Matcher matcher = java.util.regex.Pattern.compile("version (\\d+)\\.\\d+").matcher(message);
        if (matcher.find()) {
            majorVersion = Integer.parseInt(matcher.group(1));
            int compiledJavaVersion = majorVersion - 44; // JDK 1.1 = 45
            int currentJavaVersion = Runtime.version().feature();

            System.out.println("编译所用 Java 版本: Java " + compiledJavaVersion);
            System.out.println("当前 JVM 版本: Java " + currentJavaVersion);

            if (compiledJavaVersion > currentJavaVersion) {
                System.out.println("你需要使用 Java " + compiledJavaVersion + " 或更高版本来运行这个类文件。");
            }
        } else {
            System.out.println("无法解析 class 文件的版本号。");
        }
