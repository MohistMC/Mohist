...
public class CraftChunk implements Chunk {
    ...
    public CraftChunk(net.minecraft.world.chunk.WorldChunk chunk) {
        ...
        worldServer = (ServerWorld) ((ExtraWorldChunk) this).getWorld();
    ...
    public World getWorld() {
        return ((ExtraServerWorld) this).getCraftWorld();
    }
    ...
    @Override
    public CraftBlock getBlock(int x, int y, int z) {
    ...
    public Entity[] getEntities() {
        ...
        for (int i = 0; i < 16; i++) {
            count += ((ExtraWorldChunk) this).getEntitySections()[i].size();
        }
        ...
        for (int i = 0; i < 16; i++) {
            ...
            for (Object obj : ((ExtraWorldChunk) this).getEntitySections()[i].toArray()) {
    ...
    public BlockState[] getTileEntities() {
        ...
        BlockState[] entities = new BlockState[((ExtraWorldChunk) this).getBlockEntities().size()];

        for (Object obj : ((ExtraWorldChunk) this).getBlockEntities().keySet().toArray()) {
            if (!(obj instanceof BlockPos)) {
                continue;
            }

            BlockPos position = (BlockPos) obj;
            entities[index++] = ((ExtraServerWorld) this).getCraftWorld().getBlockAt(position.getX(), position.getY(), position.getZ()).getState();
        }
    ...
    public boolean contains(BlockData block) {
        ...
        for (ChunkSection section : getHandle().getSectionArray()) {
            if (section != null && section.getContainer().method_19526((Predicate<net.minecraft.block.BlockState>) nms)) {
                return true;
            }
        }
    ...
    public ChunkSnapshot getChunkSnapshot(boolean includeMaxBlockY, boolean includeBiome, boolean includeBiomeTempRain) {
        ...
        for (int i = 0; i < cs.length; i++) {
            if (cs[i] == null) { // Section is empty?
                ...
                LightingProvider lightengine = ((ExtraWorldChunk) this).getWorld().getChunkManager().getLightingProvider();
        ...
        if (includeMaxBlockY) {
            ...
            hmap.setTo(((ExtraWorldChunk) this).getHeightmaps().get(Heightmap.Type.MOTION_BLOCKING).asLongArray());
        }
    ...
    public static ChunkSnapshot getEmptyChunkSnapshot(int x, int z, CraftWorld world, boolean includeBiome, boolean includeBiomeTempRain) {
        ...
        if (includeBiome || includeBiomeTempRain) {
            ...
            biome = new BiomeArray(new ChunkPos(x, z), wcm);
        }
