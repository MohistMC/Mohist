...
public final class CraftItemStack extends ItemStack {
    ...
    public void setType(Material type) {
        if (getType() == type) {
            ...
            ((ExtraItemStack)(Object)handle).setItem(CraftMagicNumbers.getItem(type));
            if (hasItemMeta()) {
    ...
    public static boolean setItemMeta(net.minecraft.item.ItemStack item, ItemMeta itemMeta) {
        ...
        if (oldItem != newItem)
            ((ExtraItemStack)(Object)item).setItem(newItem);

        CompoundTag tag = new CompoundTag();
        item.setTag(tag);

        ((CraftMetaItem) itemMeta).applyToItem(tag);
        ((ExtraItemStack)(Object)item).convertStack(((CraftMetaItem) itemMeta).getVersion());
