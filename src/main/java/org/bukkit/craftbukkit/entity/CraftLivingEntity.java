...
public class CraftLivingEntity extends CraftEntity implements LivingEntity {
    ...
    public void setMaxHealth(double amount) {
        ...
        getHandle().getAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(amount);
    ...
    public void resetMaxHealth() {
        setMaxHealth(getHandle().getAttribute(SharedMonsterAttributes.MAX_HEALTH).getAttribute().getDefaultValue());
    }
    ...
    public <T extends Projectile> T launchProjectile(Class<? extends T> projectile, Vector velocity) {
        ...
        if (Snowball.class.isAssignableFrom(projectile)) {
            ...
            ((ThrowableEntity) launch).shoot(getHandle(), getHandle().rotationPitch, getHandle().rotationYaw, 0.0F, 1.5F, 1.0F); // ItemSnowball
        } else if (Egg.class.isAssignableFrom(projectile)) {
            launch = new EggEntity(world, getHandle());
            ((ThrowableEntity) launch).shoot(getHandle(), getHandle().rotationPitch, getHandle().rotationYaw, 0.0F, 1.5F, 1.0F); // ItemEgg
        } else if (EnderPearl.class.isAssignableFrom(projectile)) {
            launch = new EnderPearlEntity(world, getHandle());
            ((ThrowableEntity) launch).shoot(getHandle(), getHandle().rotationPitch, getHandle().rotationYaw, 0.0F, 1.5F, 1.0F); // EnderPearlItem
            ...
            ((AbstractArrowEntity) launch).shoot(getHandle(), getHandle().rotationPitch, getHandle().rotationYaw, 0.0F, 3.0F, 1.0F); // ItemBow
            ...
            ((ThrowableEntity) launch).shoot(getHandle(), getHandle().rotationPitch, getHandle().rotationYaw, -20.0F, 0.5F, 1.0F); // SplashPotionItem
        } else if (ThrownExpBottle.class.isAssignableFrom(projectile)) {
            launch = new ExperienceBottleEntity(world, getHandle());
            ((ThrowableEntity) launch).shoot(getHandle(), getHandle().rotationPitch, getHandle().rotationYaw, -20.0F, 0.7F, 1.0F); // ExperienceBottleItem
    ...
    public void swingMainHand() {
        getHandle().func_226292_a_(Hand.MAIN_HAND, true); // PAIL rename swingHand
    }

    @Override
    public void swingOffHand() {
        getHandle().func_226292_a_(Hand.OFF_HAND, true); // PAIL rename swingHand
    }
