...
public final class PaperTickList<T> extends ServerTickList<T> { // extend to avoid breaking ABI
    ...
    public void tick() {
        ...
        for (final NextTickListEntry<T> toTick : this.toTickThisTick) {
            ...
            try {
                ...
                CrashReportCategory.populateBlockDetails(crashReportCategory, toTick.getPosition(), (BlockState) null);
    ...
    @Override
    public boolean isPendingTickThisTick(final BlockPos BlockPos, final T data) {
        final ArrayList<NextTickListEntry<T>> entries = this.entriesByBlock.get(MCUtil.getBlockKey(BlockPos));
    ...
    @Override
    public boolean isScheduledForTick(final BlockPos BlockPos, final T data) {
        final ArrayList<NextTickListEntry<T>> entries = this.entriesByBlock.get(MCUtil.getBlockKey(BlockPos));
    ...
    @Override
    public void schedule(BlockPos BlockPos, T t, int i, TickPriority tickListPriority) {
        this.schedule(BlockPos, t, i + this.currentTick, tickListPriority);
    }
    ...
    @Override
    public void copy(MutableBoundingBox structureboundingbox, BlockPos BlockPos) {
        // start copy from TickListServer // TODO check on update
        ...
        while (iterator.hasNext()) {
            ...
            if (structureboundingbox.hasPoint( nextticklistentry.getPosition())) {
                BlockPos BlockPos1 = nextticklistentry.getPosition().add(BlockPos);
                T t0 = nextticklistentry.getData();

                this.schedule(new NextTickListEntry<>(BlockPos1, t0, nextticklistentry.getTargetTick(), nextticklistentry.getPriority()));
            }
        }
        // end copy from TickListServer
    }
