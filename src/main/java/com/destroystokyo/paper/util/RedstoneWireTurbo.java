...
public class RedstoneWireTurbo {
     ...
     * in order to more efficiently and deterministically compute new redstone wire power Worlds
     ...
     *   wire blocks to communicate power World changes to each other, generating 36 block
     *   updates per call.  This improved implementation propagates power World changes directly
     *   between redstone wire blocks.  Redstone wire power Worlds are therefore computed more quickly,
     *   and block updates are sent only to non-redstone blocks, many of which may perform an
     *   action when informed of a change in redstone power World.  (Note:  Block updates are not
     ...
     * - If C is in layer N of the update schedule, then all 24 positions may be scheduled for layer N1.  For redstone wire, no
     *   updates are scheduled for positions that cannot directly connect.  Additionally, the four positions above and below C
     *   are ALSO scheduled for layer N2.
    ...
    private void updateNode(final World worldIn, final UpdateNode upd1, final int layer) {
        ...
        // Ask the wire block to compute its power World from its neighbors.
        // This will also update the wire's power World and return a new
        // state if it has changed.  When a wire power World is changed,
    ...
    private void findNeighbors(final World worldIn, final UpdateNode upd1) {
        ...
        // checks for redstone wire at the same Y World and also Y1 and Y-1, relative to the
    ...
    private void propagateChanges(final World worldIn, final UpdateNode upd1, final int layer) {
        ...
        // All neighbors may be scheduled for layer N1
        ...
        // Nodes above and below are scheduled ALSO for layer N2
    ...
    // currently being processed visible at the object World.
    ...
    /*
     * Normally, when Minecraft is computing redstone wire power changes, and a wire power World
    ...
    private BlockState scheduleReentrantNeighborChanged(final World worldIn, final BlockPos pos, final BlockState newState, final BlockPos source) {
        ...
        // propagateChanges, the neighbors of 'pos' are scheduled for layers currentWalkLayer1
        // and currentWalkLayer2.
    ...
    private BlockState calculateCurrentChanges(final World worldIn, final UpdateNode upd) {
        ...
        if (k < 15) {
            ...
            for (int m = 0; m < 4; m++) {
                ...
                // Get the max redstone power World of each of the cardinal
                // neighbors
        ...
        if (i != j) {
            // If the power World has changed from its previous value, compute a new state
    ...
    /*
     * Optimized function to compute a redstone wire's power World based on cached
     * state.
     */
