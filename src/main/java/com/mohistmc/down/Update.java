...
public class Update {
    ...
    public static void hasLatestVersion() {
        ...
        try {
            ...
            String newversion = MohistConfigUtil.getUrlString(ver, MohistMC.VERSION);
            String oldversion = MohistMC.VERSION;
            if (oldver.equals(s2)) {
    ...
    public static boolean getLibrariesVersion() {
        String s = MohistMC.LIB_VERSION;
        File lib = new File(JarTool.getJarDir() + "/libraries/libraries.ver");
        if (!lib.exists()) {
            return true;
        }
        // Get the data in lib
        String i = MohistConfigUtil.getString(lib, "version:", MohistMC.LIB_VERSION);
        if (i.equals(s)) {
            return false;
        }
        return true;
