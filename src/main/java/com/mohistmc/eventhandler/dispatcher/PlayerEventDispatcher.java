...
public class PlayerEventDispatcher {
    ...
    public void onContainerClose(PlayerContainerEvent.Close event) {
        ...
        if (abstractcontainermenu.getBukkitView() == null) {
            org.bukkit.inventory.Inventory inventory = new CraftInventory(new MohistModsInventory(abstractcontainermenu, event.getPlayer()));
            inventory.getType().setMods(true);
            abstractcontainermenu.bukkitView = new CraftInventoryView(event.getPlayer().getBukkitEntity(), inventory, abstractcontainermenu);
        }
        // Mohist end
        CraftEventFactory.handleInventoryCloseEvent(event.getPlayer()); // CraftBukkit
    }
}
