...
import static com.mohistmc.network.download.NetworkUtil.getConnLength;
...
public class UpdateUtils {
    ...
    try {
      ...
      if(jar_sha.equals(build_number))
        ...
        if(bMohist("check_update_auto_download"))
          downloadFile("https://ci.codemc.io/job/Mohist-Community/job/Mohist-1.12.2/lastSuccessfulBuild/artifact/projects/mohist/build/libs/mohist-" + build_number + "-server.jar", new File(new File(MohistMC.class.getProtectionDomain().getCodeSource().getLocation().getPath().substring(1)).getName()));
      }
    } catch (Throwable e) {
  ...
  public static void downloadFile(String URL, File f) throws Exception {
    ...
    if(URL.contains("codemc.io"))
      restartServer(new ArrayList<>(Arrays.asList("java", "-jar", new File(MohistMC.class.getProtectionDomain().getCodeSource().getLocation().getPath().substring(1)).getName())));
